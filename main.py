...
col1, col2, col3, col4, col5 = st.columns([width] * 5)
...
col1, col2, col3, col4 = st.columns([width]*2 +[0.4, 0.6])  # size of columns
# Years
start_year = col1.number_input("start year ", min_value=2015,
                               max_value=2022, value=2022, step=1, key="start_year_input")
end_year = col2.number_input("end year", min_value=2015,
                             max_value=2022, value=2022,  step=1, key="end_year_input")

# Protection for order
if start_year > end_year:
    st.error("Error: Start Year is Greater Than End Year",icon="🚨")
months_input = col3.multiselect(
    'Select Months',
...
# Generate df_results
if col1.button("Day_Night"):
    a = Open_close_strategy(months=months_input,year=2022)
    st.write(months_input)
    df = a.proccess_data()
...
#                 )
if col2.button("Weekend"):
    col2.write(months_input)
